...
const TeamsPage = () => {
  ...
  return (
    <div className="container mx-auto mb-10 mt-6 px-6 bg-sky-500">
      <h1 className="text-3xl font-bold mb-4 text-center text-white">Team Inspector</h1>
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-5">
        {teamMembers.map((member, index) => (
          <div key={index} className="flex flex-col items-center">
            <div className="bg-gray-100 p-4 rounded-lg shadow-md w-full">
              <div className='relative h-16 w-16 overflow-hidden rounded-full mb-4 flex justify-center'>
                <Image 
                src={member.picture.large} alt={'${member.name.tittle} ${member.name.first} ${member.name.last}'}
                layout='fill'
                objectFit='cover'
